	...
	if ( ! empty( $poster_image ) ) {
		...
		<div class="video-social-hosted mk-center-video">
			...
			<?php if ( 'youtube' == $view_params['stream_host_website'] ) { ?>
					...
					<iframe aria-label="Youtube Video" src="https://www.<?php echo esc_attr( mk_get_thirdparty_domain_name( 'youtube' ) ); ?>/embed/<?php echo esc_attr( $view_params['stream_video_id'] ); ?>?rel=0&amp;wmode=transparent&amp;enablejsapi=1&amp;controls=0&amp;showinfo=0&amp;loop=<?php echo esc_attr( $loop ); ?>&amp;playlist=<?php echo esc_attr( $view_params['stream_video_id'] ); ?>"></iframe>

				<?php } else if ( 'vimeo' == $view_params['stream_host_website'] ) { ?>
					<?php wp_enqueue_script( 'api-vimeo' ); ?>
					<iframe aria-label="Vimeo Video" src="//player.vimeo.com/video/<?php echo esc_attr( $view_params['stream_video_id'] ); ?>?autoplay=1&badge=0&title=0&byline=0&background=1&muted=1&loop=<?php echo esc_attr( $loop ); ?>" width="500" height="281" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
				<?php } ?>
		</div>
		</div>
