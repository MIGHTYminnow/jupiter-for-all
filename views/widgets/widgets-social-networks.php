		...
		if ( ! empty( $instance['enable_sites'] ) ) {
			foreach ( $instance['enable_sites'] as $site ) {
				...
				$link = isset( $instance[ strtolower( $site ) ] ) ? $instance[ strtolower( $site ) ] : '#';
					$screen_reader_text = '<span class="screen-reader-text">' . $site . '</span>';
					$output .= '<a href="' . esc_url( $link ) . '" rel="nofollow" class="builtin-icons ' . $icon_style_css . $skin . ' ' . $path . ' ' . $site . '-hover" target="_blank" alt="' . $alt . ' ' . $site . '" title="' . $alt . ' ' . $site . '">' . Mk_SVG_Icons::get_svg_icon_by_class_name( false, $icon_name, $icon_height ) . $screen_reader_text . '</a>';
